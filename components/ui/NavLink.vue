<script setup lang="ts">
  ...
  const isActive = computed(() => {
    ...
    return (
      (route.path.startsWith(props.href) && props.href !== '/') ||
  ...
  const classes = computed(() => {
    ...
    const colorClass = isActive.value
      ? `text-${props.hoverColor} font-medium`
      : `text-${props.color} hover:text-${props.hoverColor}`
